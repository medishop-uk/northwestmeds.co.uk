...
var REVIEW_API_URL=window.NORTHWESTMEDS_DATA_API||'';
var REVIEW_SPREADSHEET_ID=window.NORTHWESTMEDS_DATA_SPREADSHEET_ID||'';
var catalog={
...
function leadMarkup(node,fallback){if(!node)return safe(fallback);var clone=node.cloneNode(true);clone.querySelectorAll('[style]').forEach(function(element){element.removeAttribute('style')});return clone.innerHTML}
function message(name,qty){return encodeURIComponent('Hello NorthwestMeds, I would like to request '+name+'. Quantity: '+qty+'. Please confirm available packs, prescription requirements and delivery options.')}
...
function key(productSlug){return 'northwestmedsReviews:'+productSlug}
function formatReviewDate(value){
 if(value===undefined||value===null||value==='')return '';
 var raw=String(value).trim(),date;
 if(/^\d+(?:\.\d+)?$/.test(raw)&&Number(raw)>20000)date=new Date((Number(raw)-25569)*86400000);
 else{date=new Date(raw);if(isNaN(date.getTime())){var m=raw.match(/^(\d{1,2})[\/.\-](\d{1,2})[\/.\-](\d{4})(?:[ ,T]+(\d{1,2}):(\d{2})(?::(\d{2}))?\s*(AM|PM)?)?/i);if(m){var a=Number(m[1]),b=Number(m[2]),day=a,month=b;if(a<=12&&b>12){month=a;day=b}var hour=Number(m[4]||0),ampm=String(m[7]||'').toUpperCase();if(ampm==='PM'&&hour<12)hour+=12;if(ampm==='AM'&&hour===12)hour=0;date=new Date(Number(m[3]),month-1,day,hour,Number(m[5]||0),Number(m[6]||0))}}}
 return date&&!isNaN(date.getTime())?date.toLocaleDateString('en-GB',{day:'numeric',month:'short',year:'numeric'}):''
}
function displayReviews(items){var list=document.querySelector('[data-review-list]');if(!list)return;var approved=(items||[]).filter(function(item){return !item.status||String(item.status).toLowerCase()==='approved'});if(!approved.length)return;list.innerHTML=approved.map(function(item){return '<article class="review-item">'+stars(Number(item.rating))+'<h3>'+safe(item.name||'Customer')+'</h3><time>'+safe(formatReviewDate(item.timestamp))+'</time><p>'+safe(item.review)+'</p></article>'}).join('')}
...
function init(){
 ...
 var leadNode=hero&&hero.querySelector('.page-hero-lead'),fallbackLead='Review information, availability and prescription requirements for '+defaults[0]+'.';
 var lead=text(leadNode?leadNode.textContent:fallbackLead),leadHtml=leadMarkup(leadNode,fallbackLead);
 var brand=field('Brand Name',article)||defaults[1],manufacturer=field('Manufacturer',article)||defaults[2],vendor=field('Vendor',article)||'NorthwestMeds',category=field('Categories',article)||field('Category',article)||defaults[3];
