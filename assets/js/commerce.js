...
var DATA_API_URL=window.NORTHWESTMEDS_DATA_API||'';
var DATA_SPREADSHEET_ID=window.NORTHWESTMEDS_DATA_SPREADSHEET_ID||'';
var pricing={
...
var cart=[];
try{cart=JSON.parse(localStorage.getItem('northwestmedsCart')||'[]')}catch(error){cart=[]}
function money(value){return '£'+Number(value).toFixed(0)}
function saveCart(){localStorage.setItem('northwestmedsCart',JSON.stringify(cart));renderCart();updateCount()}
...
function newOrder(channel){
 ...
 return {orderId:'NWM-'+Date.now().toString(36).toUpperCase()+'-'+Math.random().toString(36).slice(2,6).toUpperCase(),timestamp:new Date().toISOString(),channel:channel,status:'New',items:cart,subtotal:subtotal,postage:postage,total:subtotal+postage,currency:'GBP',pageUrl:location.href,utmSource:new URLSearchParams(location.search).get('utm_source')||'',utmMedium:new URLSearchParams(location.search).get('utm_medium')||'',utmCampaign:new URLSearchParams(location.search).get('utm_campaign')||''}
}
async function checkout(channel){
 var status=document.querySelector('[data-checkout-status]');if(!cart.length)return;var order=newOrder(channel);
 if(!DATA_API_URL){location.href=channel==='whatsapp'?whatsapp+'?text='+encodeURIComponent(orderMessage(order,channel)):telegram;return}
 status.textContent='Saving your request securely…';document.querySelectorAll('[data-checkout]').forEach(function(button){button.disabled=true});
 try{
